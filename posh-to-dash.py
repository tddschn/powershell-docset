...
class Configuration:
    ...
    def __init__(self, args):
        ...
        self.build_folder = os.path.join(
            os.getcwd(), "_build_{0:s}".format(self.powershell_version)
        )
...
def download_page_contents(configuration, uri, output_filepath):
    ...
    versionned_url = "{0:s}?{1:s}".format(
        full_url, configuration.powershell_version_param
    )

    download_textfile(versionned_url, output_filepath)


def download_module_contents(
    configuration, module_name, module_uri, module_dir, cmdlets, root_dir
):
    ...
    logging.debug(
        "downloading %s module index page  -> %s" % (module_name, module_filepath)
    )
    if module_uri:
    ...
    for cmdlet in cmdlets:
        ...
        logging.debug(
            "downloading %s cmdlet doc -> %s" % (cmdlet_name, cmdlet_filepath)
        )
...
def crawl_posh_contents(
    ...
    logging.debug(
        "raw modules : %s"
        % [m['toc_title'] for m in modules_toc['items'][0]['children']]
    )
    ...
    if len(configuration.filter_modules):
        modules = list(
            filter(
                lambda m: m['toc_title'].lower() in configuration.filter_modules,
                modules,
            )
        )
    ...
    for module in modules:
        ...
        module_infos = download_module_contents(
            configuration,
            module_name,
            module_uri,
            module_dir,
...
def rewrite_soup(
    configuration: Configuration, soup, html_path: str, documents_dir: str
):
...
def rewrite_index_soup(
    configuration: Configuration, soup, index_html_path: str, documents_dir: str
):
    """rewrite html contents by fixing links and remove unnecessary cruft"""

    # Fix navigations links
    content_tables = soup.findAll("table", {"class": "api-search-results"})
    ...
    for content_table in content_tables:
        ...
        for image in images:
            image['src'] = os.path.relpath(
                module_svg_path, os.path.dirname(index_html_path)
            )
...
def download_additional_resources(
    ...
    icon_module_url = '/'.join(
        ["https:/", Configuration.domain, "en-us", "media", "toolbars", "module.svg"]
    )
    icon_module_path = os.path.join(
        documents_dir, Configuration.domain, "en-us", "media", "toolbars", "module.svg"
    )
    ...
    def insert_into_sqlite_db(cursor, name, record_type, path):
        ...
        try:
            ...
            if dbpath is None and dbname is None:
                ...
                logging.debug(
                    'DB add [%s] >> name: %s, path: %s' % (record_type, name, path)
                )
            else:
...
def merge_folders(src, dst):
    ...
    if os.path.isdir(src):
        ...
        for name in os.listdir(src):
            merge_folders(os.path.join(src, name), os.path.join(dst, name))
    else:
...
def main(configuration: Configuration):
    ...
    additional_resources_dir = os.path.join(
        configuration.build_folder, "_3_additional_resources"
    )
    ...
    content_toc = crawl_posh_contents(
        configuration, configuration.docs_toc_url, download_dir
    )
    ...
    if os.path.exists(os.path.join(win10_download_dir, "toc.json")):
        ...
        windows_toc = crawl_posh_contents(
            configuration, configuration.windows_toc_url, win10_download_dir
        )
    ...
    download_additional_resources(
        configuration, additional_resources_dir, resources_to_dl
    )
    ...
    shutil.copy(
        os.path.join(src_dir, "static/DASH_LICENSE"),
    ...
    make_docset(docset_dir, configuration.output_filepath, Configuration.docset_name)
